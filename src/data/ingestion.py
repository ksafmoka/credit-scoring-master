...
def load_lending_club_data(
    ...
    # --- Dry-run first chunk BEFORE truncate (never wipe DB if CSV is unusable) ---
    read_kw = dict(chunksize=chunk_size, low_memory=False)
    try:
        probe_reader = pd.read_csv(filepath, on_bad_lines="skip", **read_kw)
    except TypeError:
        probe_reader = pd.read_csv(filepath, **read_kw)

    first_ok = None
    for probe_chunk in probe_reader:
        try:
            first_ok = _preprocess_raw_chunk(probe_chunk)
        except Exception as exc:
            logger.warning(f"Probe chunk failed preprocess: {exc}")
            first_ok = pd.DataFrame()
        if first_ok is not None and not first_ok.empty:
            break
    else:
        raise ValueError(
            f"No usable rows in {filepath} (all chunks empty after preprocess). "
            "Check columns: loan_amnt/loan_amount, annual_inc/income, "
            "issue_d, loan_status."
        )

    logger.info(
        f"Probe OK: first usable chunk has {len(first_ok)} rows. "
        f"Columns sample: {list(first_ok.columns)}"
    )
    ...
    if total_loaded == 0:
        raise ValueError(
            f"No usable rows loaded from {filepath} after truncate. "
            "DB may be empty — fix CSV and re-run data_ingestion."
        )
    return total_loaded
