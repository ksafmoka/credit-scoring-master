...
class Predictor:
    ...
    def _compute_features(self, request: ScoringRequest, seg_model: _SegmentModel) -> pd.DataFrame:
        ...
        # New features
        features["loan_term"] = float(request.loan_term)
        # num_inquiries_6m not in ScoringRequest — will be filled by medians
        features["num_inquiries_6m"] = 0.0
        ...
        features["dti_x_credit_score"] = float(
            request.dti_ratio * np.clip((request.credit_score - 300) / 550.0, 0, 1)
        )
        features["loan_amount_x_interest_rate"] = float(
