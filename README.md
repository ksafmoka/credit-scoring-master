...
| 02 | [`04_threshold_analysis.ipynb`](notebooks/04_threshold_analysis.ipynb) | PR curve, threshold selection (F1-optimal + cost-based), production recommendations |
| 03 | [`05_error_analysis.ipynb`](notebooks/05_error_analysis.ipynb) | Error analysis by segments, FP vs FN patterns, calibration, edge cases |
...
|----------|-------------|
| [Architecture](docs/architecture.md) | System design, data flow, dual-model details, health checklist |
| [Model Card](docs/model_card.md) | Model details, metrics, limitations, ethics |
| [Feature Documentation](docs/feature_documentation.md) | All features with formulas |
...
| Decision | Why |
|----------|-----|
| Dual-model serving | Banks use separate models for thick/thin-file |
| fe_ids consistency | All FE tasks use same IDs → no merge mismatches |
| Optuna on segment data | Params optimized per-segment, not on full dataset |
| Conditional calibration | Only calibrate if it improves AUC |
| Time-window statistical proof | Chi-squared, KS, PSI tests justify 2015-2018 window |
| Sequential DAG tasks | Avoids Postgres shared memory exhaustion on weak PC |
| Named Docker volume | Avoids bind mount permission issues |
